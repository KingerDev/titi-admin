...
function submitPush() {
    pushForm.post(route('campaign-pushes.store', props.campaign.id), {
        onSuccess: () => {
...
function cancelPush(push) {
    ...
    router.delete(route('campaign-pushes.destroy', { campaignId: props.campaign.id, pushId: push.id }), {
...
async function loadStats(push) {
    ...
    try {
        const res  = await fetch(route('campaign-pushes.stats', { campaignId: props.campaign.id, pushId: push.id }));
        const data = await res.json();
        pushStats.value[push.id] = data;
    } catch (_) {}
}

props.pushes?.forEach(p => { if (p.onesignal_id) loadStats(p); });
...
const statusLabel = { pending: 'Čaká', sent: 'Odoslaná', cancelled: 'Zrušená' };
const statusClass  = {
...
const campaignStatusLabel = { draft: 'Draft', active: 'Aktívna' };
const campaignStatusClass  = { draft: 'bg-gray-100 text-gray-600', active: 'bg-green-100 text-green-700' };
</script>
        ...
        <template #header>
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                    <a :href="route('campaigns.edit', campaign.id)" class="text-gray-400 hover:text-gray-600 transition-colors">
                    ...
                    <div>
                        <div class="flex items-center gap-2">
                            <h2 class="text-xl font-semibold leading-tight text-gray-800">Push notifikácie</h2>
                            <span :class="campaignStatusClass[campaign.status]"
        ...
        <div class="py-8">
            <div class="mx-auto max-w-4xl px-4 sm:px-6 lg:px-8">
                ...
                <div v-if="!pushes?.length"
                    ...
                    <p class="text-sm font-medium text-gray-500">Žiadne push notifikácie</p>
                    <p class="mt-1 text-xs text-gray-400 mb-4">Kampaň zatiaľ nebola odoslaná žiadnemu segmentu.</p>
                    <button @click="openModal"
                            class="inline-flex items-center gap-2 rounded-md bg-indigo-600 px-4 py-2 text-sm font-medium text-white hover:bg-indigo-700 transition-colors">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/>
                ...
                <div v-else class="space-y-3">
                    <div v-for="push in pushes" :key="push.id"
                        ...
                        <div class="flex items-start justify-between gap-4">
                            <div class="space-y-2 min-w-0">
                                ...
                                <!-- Stats -->
                                <div v-if="pushStats[push.id]" class="flex items-center gap-4 pt-1">
                            ...
                            <div class="flex flex-col items-end gap-2 shrink-0">
                                <span :class="statusClass[push.status]"
                ...
                <div v-if="showModal"
                    ...
                    <div class="w-full max-w-md rounded-xl bg-white shadow-xl">
                        <div class="flex items-center justify-between border-b border-gray-100 px-6 py-4">
                            <h3 class="text-base font-semibold text-gray-900">Nová push notifikácia</h3>
